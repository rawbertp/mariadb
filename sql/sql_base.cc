                           ...
                           uint8 op)
{
  ...
  table->fk_ref_list= new(thd->mem_root) List<FOREIGN_KEY_INFO>();
  table->file->get_parent_foreign_key_list(thd, table->fk_ref_list);
  ...
  *need_prelocking= TRUE;
  ...
  List_iterator<FOREIGN_KEY_INFO> fk_list_it(*table->fk_ref_list);
  while ((fk= fk_list_it++))
  {
