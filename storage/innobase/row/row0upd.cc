	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	const rec_t*	rec;
	dberr_t		err;
	row_prebuilt_t* prebuilt = node->prebuilt;
	...
	ut_ad(prebuilt->m_mysql_table->fk_ref_list);
	ut_ad(prebuilt->m_mysql_table->fk_ref_list->elements
		== table->referenced_set.size());
        auto ref_table_it= prebuilt->m_mysql_table->fk_ref_list->begin();
	for (dict_foreign_set::iterator it = table->referenced_set.begin();
		...
		if (foreign->referenced_index == index
			...
			/* This lock type is chosen for cascade changes */
                        if (ref_table_it->table_list->lock_type
							== TL_FIRST_WRITE)
				ref_prebuilt=
